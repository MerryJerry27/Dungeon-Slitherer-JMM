...
class Entity(pygame.sprite.Sprite):
    def __init__(self, type, x, y, image_width, image_height, entity_width, entity_height, display, extra_info):
        ...
        # IMAGE, ROTATION, AND EXTRA INFO CALCULATION
        if self.type == "Panel":
        ...
        """
        Create an Entity sprite. These entities can be any size now.
        ...
        """
        self.image = self.create_image(os.path.join("assets", self.stored_image), self.stored_rotation, image_width, image_height, entity_width, entity_height)
        ...
        type determines what the button does
            INFO FOR EVENT
            "Level": (int level)
            determines what level to go to when clicked
        ...
        allows which direction(s) you can enter and exit

        "Enemy" : (int damage, boolean contact_death, int move_by, int speed, int last_move, coord-list starting_position, (list stages: int move_stages, int cycle_stage), (list move_pattern: movement-list direction, int distance)
        deals damage on contact, determined if contact_death, move_by pixels per speed seconds, travel move_pattern hold lines to follow, move_stages and last_move stores info needed to calculate
        move_pattern is a list of paths, with a path formatted as ((coord-list direction), move_amount) (eg. ((1, 0), 3))
        move_stages formatted as (a, b), where 0 >= a < number of move_patterns, and 0 >= b < move_amount in the given stage, both used to track at what part of move_pattern the enemy is in
        '''
    ...
    def create_image(self, image_location, rotation, image_width, image_height, entity_width, entity_height):
        ...
        # But now it can tile in both directions! (but for some reason makes transparency black, so that's unfortunate) 
