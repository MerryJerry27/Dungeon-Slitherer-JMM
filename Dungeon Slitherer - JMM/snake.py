...
class SnakeHitbox(pygame.sprite.Sprite):
    def __init__(self, x, y):
        super().__init__()
        image_location = os.path.join("Dungeon Slitherer - JMM", "assets", "hitbox.png")
...
class SnakeBody(pygame.sprite.Sprite):
    def __init__(self, x, y, front_direction, back_direction):
        super().__init__()
        image_location = os.path.join("Dungeon Slitherer - JMM", "assets", "snake_body_straight.png")
        image_location_two = os.path.join("Dungeon Slitherer - JMM", "assets", "snake_body_corner.png")
        image_location_three = os.path.join("Dungeon Slitherer - JMM", "assets", "snake_body_end.png")
        image_location_four = os.path.join("Dungeon Slitherer - JMM", "assets", "snake_body_straight_hurt.png")
        image_location_five = os.path.join("Dungeon Slitherer - JMM", "assets", "snake_body_corner_hurt.png")
        image_location_six = os.path.join("Dungeon Slitherer - JMM", "assets", "snake_body_end_hurt.png")
...
class Snake(pygame.sprite.Sprite):
    def __init__(self, x, y, update_every, damage_time, limit):
        super().__init__()
        image_location = os.path.join("Dungeon Slitherer - JMM", "assets", "snake_head.png")
        image_location_two = os.path.join("Dungeon Slitherer - JMM", "assets", "snake_head_hurt.png")
    ...
    def move(self, head_colliding, body_colliding, entity_colliding, x, y):
        ...
        if move_time - self.last_moved > self.update_every: # MOVE SPEED
    ...
    def follow(self):
        body_part_index = 0
        for body_part in self.snake_body: # MOVES EACH BODY PART TO LAST LOCATION, FOLLOWING SNAKE HEAD
            ...
            if len(self.snake_body.sprites()) == 1: # ONLY ONE BODY PART, LINE UP TO HEAD
                ...
                if body_part.front_direction != body_part.back_direction:
    ...
    def set_length(self, amount):
        if amount > 0 and amount <= self.limit + 1:
            if amount > len(self.snake_body.sprites()) + 1:
                while amount > len(self.snake_body.sprites()) + 1:
