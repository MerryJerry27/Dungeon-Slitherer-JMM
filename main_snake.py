...
# CAPTION / ICON SETUP
icon = os.path.join("assets", "door.png")
...
# PRODUCTION TOOL / ENTITY BUILDER FUNCTIONS
part = ""
coords = [(0, 0), (0, 0)]
...
while True:
    ...
    if control_in_level:
        ...
        # PRODUCTION TOOL / ENTITY BUILDER HOTKEYS
            ...
            if event.button == 1: # LEFT CLICK RELEASE
                    ...
                    for entity in levels[level_num].entity_list:
                        if entity.rect.collidepoint(mouse_pos):
                            if entity.type == "Button":
                                if entity.event == "Pause": # PAUSE GAME
                                    control_in_level = False
                                    control_in_menu = True
                ...
                if control_in_menu:
                    for entity in menus[menu_num].entity_list:
                        if entity.rect.collidepoint(mouse_pos):
                            if entity.type == "Button":
                                if entity.event == "Level": # GO TO DESIGNATED LEVEL
                                    ...
                                    head_collision = pygame.sprite.spritecollide(snake, levels[level_num].entity_list, False)
                                if entity.event == "Next": # GO TO NEXT LEVEL WHEN COMPLETE
                                    if level_num < 6:
                                        control_in_level = True
                                        control_in_menu = False
                                        level_num += 1
                                        for body_part in snake.snake_body:
                                            ...
                                            body_part.front_direction = snake.direction
                                            body_part.back_direction = snake.direction
                                    else: # BACK TO MAIN MENU IF AT LAST LEVEL
                                        control_in_level = False
                                        control_in_menu = True
                                        level_num = 0
                                        menu_num = 0
                                    ...
                                    if level_num != 0:
                                        for entity_2 in levels[level_num].entity_list:
                                            ...
                                            if entity_2.type == "Enemy":
                                                ...
                                                entity_2.move_stages = [0, 0]
                                    head_collision = pygame.sprite.spritecollide(snake, levels[level_num].entity_list, False)
                                if entity.event == "Resume": # UNPAUSE (USES STORED TIME SO THAT THE COOLDOWNS CARRY OVER)
                                    control_in_level = True
                                    control_in_menu = False
                                    ...
                                    head_collision = pygame.sprite.spritecollide(snake, levels[level_num].entity_list, False)
                                if entity.event == "Restart": # RESTART LEVEL
                                    control_in_level = True
                                    control_in_menu = False
                                    snake.rect.x = levels[level_num].starting_x
                                    snake.rect.y = levels[level_num].starting_y
                                    ...
                                    for entity_2 in levels[level_num].entity_list:
                                        ...
                                        if entity_2.type == "Enemy":
                                            ...
                                            entity_2.move_stages = [0, 0]
                                    head_collision = pygame.sprite.spritecollide(snake, levels[level_num].entity_list, False)
                                if entity.event == "Exit": # GO BACK TO MAIN MENU
                                    level_num = 0
                                    menu_num = 0
                                    snake.rect.x = levels[level_num].starting_x
                                    snake.rect.y = levels[level_num].starting_y
    ...
    # ALWAYS UPDATE THE SNAKES (BOOLEAN FOR DAMAGE COOLDOWN)
    snake.update(control_in_level)
    snakes_health.update(True)
    ...
    if control_in_level:
        ...
        for entity in levels[level_num].entity_list:
                if entity.type == "Number":
                    if entity.linked_to == "Snake_Length": # HEALTH COUNTER UPDATE
                ...
                if entity.type == "Enemy": # ENEMY MOVEMENT
                    if entity.display:
                        enemy_current_time = pygame.time.get_ticks()
                        if enemy_current_time - entity.last_move > entity.speed: # MOVE WHEN TIME HITS SPEED AMOUNT
                            # USES STAGES TO FOLLOW THEIR PATHS
            ...
            if head_collision[0].type == "Enemy":
                ...
                if enemy_damage_current_time - snake.last_damage > snake.damage_time:
                    if head_collision[0].display:
                        ...
                        if len(snake.snake_body.sprites()) > 0: # LOSE HEALTH WHEN POSSIBLE
                            snake.change_length(head_collision[0].damage)
                        else: # DIE WITH ONLY HEAD LEFT
                            control_in_level = False
                            control_in_menu = True
                            snake_health.rect.x = -40
                            snake_health.rect.y = -40
        ...
        for body_part_collision in snake_body_collisions:
            if len(body_part_collision) > 0:
                if body_part_collision[0].type == "Enemy": # NO DEATH CHECK SINCE ALL BODY PARTS WOULD BE GONE
                    enemy_damage_current_time = pygame.time.get_ticks()
                    if enemy_damage_current_time - snake.last_damage > snake.damage_time:
